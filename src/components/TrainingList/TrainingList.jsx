    ...
    return trainings.map((training) => (
      <article className="section_training_card" key={training._id}>
        ...
        {training.musculos.map((musculo, id) => {
                        return <span className="span" key={id}><img className="tick" src={tick} alt="tick"></img>{musculo}</span>
                    })}
        <p className="description">{training.descripcion}</p>
        <p className="element"><strong>Dificultad:</strong> {training.dificultad}</p>
        <p className="element"><strong>Series:</strong> {training.series}</p>
        <p className="element"><strong>Repeticiones:</strong> {training.repeticiones}</p>
      </article>
    ));
  };
